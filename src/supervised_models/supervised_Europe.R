# model training for Europe keywords
...
keep <- which(corp_features$Europe == 1 | corp_features$Europe == 0)
features <- corp_features[keep,]
dtm <- dtm[keep,]

# partition our data into train and test sets:
trainIndex <- createDataPartition(features$Europe,
                                  p = 0.8,
                                  list = FALSE,
                                  times = 1)
...
train.label  <- features$Europe[trainIndex]
test.label   <- features$Europe[-trainIndex]
...
Europe_top <- as.matrix(Marranged_Europe)
Europe_top <- Europe_top[1:100,1]

Europe2_top <- as.matrix(Marranged2_Europe)
Europe2_top <- Europe2_top[1:100,1]

Europetopfinal <- c(Europe_top, Europe2_top)
